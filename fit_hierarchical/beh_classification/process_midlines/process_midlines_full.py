...
for k, exp_date in enumerate(exp_dates): 
    ...
    '''
    from pathlib import Path
    params = yaml.safe_load(Path(exp_params_yml).read_text())
    if exp_date in params:
        exp_params = params[exp_date]
    else: 
        exp_params = {
        }
    if "midline_flipped" not in exp_params:
        exp_params["midline_flipped"] = False
    if "mask_start_end_is" not in exp_params: 
        exp_params["mask_start_end_is"] = []
    if "a_to_p" not in exp_params:
        exp_params["a_to_p"] = 6
    if "midline_flipped_frames" not in exp_params:
        midline_flipped_frames = None
    else: 
        n_frames = nir_to_confocal.shape[0]
        midline_flipped_frames =  [list(range(item['start'], min(item['end'] + 1,n_frames ))) for item in exp_params['midline_flipped_frames']]
    ...
    a_to_p_thresh = exp_params["a_to_p"]
    print("a_to_p_thresh", a_to_p_thresh)
    ...
    is_looping_bin_img_confocal, a_to_ps, areas = get_is_looping_from_mask_dir_confocal(n_frames_nir, 
                                            n_frames_confocal
                                            ,nir_to_confocal, bin_img_dir, area_to_perimeter_ratio_thres = 1, area_to_perimeter = a_to_p_thresh)
    ...
    midlines = get_midlines_from_worm_tracer_csv(raw_nir_midlines_dir, 
                                                 is_midline_flipped = is_midline_flipped, 
                                                 midline_flipped_frames =midline_flipped_frames )
    ...
    # target_lengths = list(range(50,155, 10))[::-1]
    target_lengths = [110]
    ...
    for target_length in target_lengths:#, 100, 115]:
        ...
        if components is None: 
            print("no target lenth midlines, targetlegnth: " , target_length)
            continue
        ...
        if save_csvs:
            ...
            csv_name = os.path.join(csv_save_dir, f"target_length_{target_length}_eigenworm_components_nir.csv")
            np.savetxt(csv_name,  components, delimiter=",", fmt='%f')
            
            csv_name = os.path.join(csv_save_dir, f"target_length_{target_length}_cropped_midlines_wormtracer_nir.csv")
            save_as_csv(cropped_midlines, csv_name)
        ...
        masked_img_output_dir = make_new_folder(os.path.join(img_save_dir, f"bin_imgs_resampled_timebin_{time_bins}"))
        ...
        if save_csvs:
            midlines_reconstructed_resampled1 =  evenly_sample_feature(reconstructed_midlines, 
                                    nir_to_confocal,
                                    time_bins=time_bins, )
            ...
            midlines_worm_tracer_resampled1 =  evenly_sample_feature(cropped_midlines, 
                                    nir_to_confocal,
                                    time_bins=time_bins, )  
   
            
            
            csv_name = os.path.join(csv_save_dir, f"midlines_reconstructed_resampled_target_length_{target_length}_timebin_{time_bins}.csv")
            save_as_csv(midlines_reconstructed_resampled1, csv_name)
            ...
            csv_name = os.path.join(csv_save_dir, f"cropped_midlines_wormtracer_target_length_{target_length}_timebin_{time_bins}.csv")
            save_as_csv(midlines_worm_tracer_resampled1, csv_name)
